...
@section('content')
    ...
    <div class="col-lg-6">
        ...
        @endif
        <form action="{{ url('author/post/edit/'.$data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <select class="form-control" name="category_id" id="category">
                @foreach ($category as $cat)
                    <option value="{{$cat->id}}" {{ ($cat->id == $data->categories_id) ? 'selected' : '' }}>{{$cat->category_id}}</option>
                @endforeach
            </select>
